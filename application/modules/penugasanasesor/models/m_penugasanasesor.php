<?php
class M_penugasanasesor extends CI_Model{
  ...
  public function get_data_lowongan($tabel,$where=NULL){ //ambil data user dan role
    $this->db->select('*');
    $this->db->from($tabel);
    $this->db->join('lowongan_asesi', 'lowongan_asesi.id_lowongan='.$tabel.'.id_lowongan');
    if ($where != NULL) {
      $this->db->where($where);
    }
    return $query = $this->db->get();
  ...
  public function get_data_pendaftaran($tabel,$where=NULL){ //ambil data user dan role
    $this->db->select('*');
    $this->db->from($tabel);
